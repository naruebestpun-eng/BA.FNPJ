<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('เพิ่มรายวิชา') }}</h2>
    </x-slot>
            ...
            <div class="bg-white p-6 shadow-sm sm:rounded-lg">
                <form method="POST" action="{{ route('subjects.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <input name="code" value="{{ old('code') }}" class="mt-1 block w-full border-gray-300 rounded-md p-2">
                        @error('code')<p class="text-red-500 text-xs mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700">ชื่อวิชา</label>
                        <input name="name" value="{{ old('name') }}" class="mt-1 block w-full border-gray-300 rounded-md p-2">
                    ...
                    <div class="mb-4 grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="credits" value="{{ old('credits') }}" type="number" min="0" class="mt-1 block w-full border-gray-300 rounded-md p-2">
                        ...
                        <div>
                            ...
                            <input name="description" value="{{ old('description') }}" class="mt-1 block w-full border-gray-300 rounded-md p-2">
                        </div>
                    </div>
