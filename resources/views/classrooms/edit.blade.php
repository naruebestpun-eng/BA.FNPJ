            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('classrooms.update', $classroom->id) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="room_name" class="block mt-1 w-full" type="text" name="room_name" :value="old('room_name', $classroom->room_name)" required />
                        <x-input-error :messages="$errors->get('room_name')" class="mt-2" />
                    </div>

                    <div class="mb-4">
                        <x-input-label for="building" :value="__('อาคาร / สถานที่')" />
                        <x-text-input id="building" class="block mt-1 w-full" type="text" name="building" :value="old('building', $classroom->building)" />
                    ...
                    <div class="flex items-center justify-end mt-4">
                        ...
                        <x-primary-button>{{ __('อัปเดตข้อมูล') }}</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
